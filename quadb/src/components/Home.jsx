...
import { ScrollArea } from "@/components/ui/scroll-area";
import { Separator } from "@/components/ui/separator";
import { ListTodo, CalendarDays, Star, UserSquare2, Menu, Plus, ChevronDown } from "lucide-react";
import avatar from "../assets/image.png";
...
export default function Home() {
  ...
  const handleAddTask = () => {
    if (newTask.trim() !== "" && taskDate !== "") {
      setPendingTasks([...pendingTasks, { text: newTask.trim(), date: taskDate }]);
      setNewTask("");
      setTaskDate("");
    }
  };
  ...
  return (
    <div className="flex flex-col min-h-[900px] bg-[#fbfdfb]">
      ...
      <div className="flex flex-1">
        <aside className="w-[280px] bg-[#eef6ef] p-4 flex flex-col gap-4">
          <div className="text-center">
            <img src={avatar} alt="User Avatar" className="w-16 h-16 rounded-full mx-auto mb-2" />
            <p className="font-medium text-[#1a271b]">Hey, ABCD</p>
          </div>
          ...
          <nav className="space-y-1">
            {navigationItems.map((item, index) => (
              <Button
                key={index}
                variant={item.active ? "secondary" : "ghost"}
                className={`w-full justify-start gap-4 ${item.active ? "bg-[#34783729] text-[#347937]" : ""}`}
              >
                {item.icon}
                {item.label}
              </Button>
          ...
          <ScrollArea className="flex-1">
            {pendingTasks.map((task, index) => (
              <div key={index} className="flex items-center justify-between py-4 border-t">
                <div className="flex items-center gap-4">
                  <Checkbox onChange={() => handleTaskCompletion(task)} />
                  <span className="text-[#1a271b]">{task.text} ({task.date})</span>
                </div>
                <Star className="w-6 h-6" />
              </div>
            ))}
            ...
            {completedTasks.map((task, index) => (
              <div key={index} className="flex items-center justify-between py-4 border-t">
                <div className="flex items-center gap-4">
                  <Checkbox checked onChange={() => handleTaskRevert(task)} />
                  <span className="text-[#1a271b] line-through">{task.text} ({task.date})</span>
                </div>
                <Star className="w-6 h-6" />
              </div>
            ))}
